...
{
    public function markUnderReview(IdentityVerificationSubmission $submission, ?User $admin): IdentityVerificationSubmission
    {
        return $this->transition($submission, $admin, 'under_review', 'Identity verification moved under review.');
    }

    public function approve(IdentityVerificationSubmission $submission, ?User $admin, ?string $note = null): IdentityVerificationSubmission
    {
        return $this->transition($submission, $admin, 'approved', $note ?: 'Identity verification approved.');
    }

    public function reject(IdentityVerificationSubmission $submission, ?User $admin, string $reason): IdentityVerificationSubmission
    {
        return $this->transition($submission, $admin, 'rejected', $reason);
    }

    public function requestAdditionalDocument(IdentityVerificationSubmission $submission, ?User $admin, string $note): IdentityVerificationSubmission
    {
        return $this->transition($submission, $admin, 'additional_document_required', $note);
    }

    private function transition(IdentityVerificationSubmission $submission, ?User $admin, string $status, string $note): IdentityVerificationSubmission
    {
        return DB::transaction(function () use ($submission, $admin, $status, $note) {
            ...
            DB::afterCommit(function () use ($submission, $admin, $status, $note) {
                $fresh = $submission->fresh(['user', 'reviewer']);
            ...
            return $submission->fresh(['user', 'reviewer']);
        });
    }
}
