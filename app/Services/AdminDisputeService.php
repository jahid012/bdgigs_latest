...
class AdminDisputeService
{
    ...
    public function update(Dispute $dispute, User $actor, array $payload): Dispute
    {
        return DB::transaction(function () use ($dispute, $actor, $payload) {
            ...
            DB::afterCommit(function () use ($dispute, $actor, $previous, $payload) {
                $fresh = $dispute->fresh(['order.buyer', 'order.seller', 'openedBy']);
    ...
    public function join(Dispute $dispute, User $admin, ?string $note = null): Dispute
    {
        return DB::transaction(function () use ($dispute, $admin, $note) {
            ...
            $dispute->forceFill([
                ...
                'assigned_to_id' => $dispute->assigned_to_id ?: $admin->id,
            ])->save();
            ...
            return $dispute->fresh(['assignedTo', 'activities.actor']);
        });
    }

    public function requestEvidence(Dispute $dispute, User $admin, ?User $recipient, string $note): Dispute
    {
    ...
    public function issueRefund(Dispute $dispute, User $admin, int $amountCents, ?string $reason = null): Order
    {
