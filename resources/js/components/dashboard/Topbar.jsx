...
const defaultProfileLinks = [
  ...
  { label: "Billing", href: "/dashboard/payments" },
];
...
function Topbar({
  ...
  useDismissOnInteractOutside(topbarActionsRef, openMenu !== null, closeMenus);
  ...
  return (
    <header className="dashboard-topbar">
      ...
      <form className="topbar-search" role="search" aria-label="Search dashboard" onSubmit={(event) => event.preventDefault()}>
        <Icon name="search" />
        <label className="sr-only" htmlFor="dashboardSearch">
          Search dashboard
        </label>
        <input id="dashboardSearch" type="search" placeholder={searchPlaceholder} autoComplete="off" />
      </form>
      ...
      <div className="topbar-actions" ref={topbarActionsRef}>
        <div className="topbar-menu">
          ...
          <div className={`topbar-dropdown feed-dropdown${openMenu === "notifications" ? " is-open" : ""}`}>
            <div className="topbar-dropdown-heading">
              <div>
                <span>Notifications</span>
                <strong>Latest updates</strong>
              </div>
              <span className="status-badge status-completed">{notificationItems.length} new</span>
            </div>
            <div className="topbar-feed-list">
              {notificationItems.map((item) => (
                <article className="topbar-feed-item" key={`${item.title}-${item.time}`}>
                  <span className="topbar-feed-icon" aria-hidden="true">
                    <Icon name="bell" />
                  </span>
                  <span>
            ...
            <button className="topbar-dropdown-action" type="button" onClick={closeMenus}>
        ...
        <div className="topbar-menu">
          ...
          <div className={`topbar-dropdown feed-dropdown${openMenu === "messages" ? " is-open" : ""}`}>
            <div className="topbar-dropdown-heading">
              <div>
                <span>Messages</span>
                <strong>Recent conversations</strong>
              </div>
              <span className="status-badge status-delivered">{messageItems.length} threads</span>
            </div>
            <div className="topbar-feed-list">
              {messageItems.map((item) => (
                <article className="topbar-feed-item message-feed-item" key={`${item.name}-${item.time}`}>
            ...
            <button className="topbar-dropdown-action" type="button" onClick={openMessagesPage}>
              {messageActionLabel}
            </button>
          </div>
        </div>
        ...
        <div className="profile-menu">
          ...
          <div className={`profile-dropdown${openMenu === "profile" ? " is-open" : ""}`}>
            {profileLinks.map((link) => (
              <Link to={link.href} key={link.label} onClick={closeMenus}>
                {link.label}
              </Link>
            ))}
