...
export const ahmadProfile = {
  ...
  portfolio: {
    ...
    thumbnails: ["/assets/img/gig_images/20.png", "/assets/img/gig_images/21.png", "/assets/img/gig_images/1.png"],
    tags: ["Educacao", "Empresas do Setor Publico", "Tecnologia", "Tecnologia da Informacao (TI)", "Desenvolvimento de Software", "+3"],
    cost: "$800-$1000",
    duration: "1-3 months",
  },
  workExperience: [
...
export function getUserProfile(username) {
  ...
  const explicit = explicitProfiles.find((profile) => profile.slug === slug || slugifySellerName(profile.name) === slug);
  ...
  const sellerGig = listingGigs.find((gig) => slugifySellerName(gig.seller) === slug);
  if (sellerGig) {
...
function createProfileFromDetail(detail) {
  return {
    ...
    languages: detail.seller.languages.split(",").map((item) => item.trim()),
...
function createProfileFromGig(gig) {
  return {
    ...
    portfolio: {
      ...
      description: gig.title,
      image: gig.image,
      thumbnails: [gig.image, "/assets/img/gig_images/20.png", "/assets/img/gig_images/21.png"],
